...
def mostrar_tabla_financiera(df, columnas_meses):
    ...
    df.select_dtypes(include="int64").astype(str)
)
    # ---- ORDENAR PRIORIDAD ----
    ...
    def colorear_mes(valor):
        ...
        if isinstance(valor, str):
            ...
            if valor.startswith("("):
                return "background-color: #fff3cd; color: #721c24; font-weight: bold;"
...
df_rec2= get_gsheet_df(
    sheet_id=SHEET_ID_FAC,
    worksheet_name="PANEL"
)
...
subheader_custom("Evolutivo Indicadores Generales", size=30)
mostrar_tabla_financiera(df_rec2, columnas_meses)
st.markdown("---")

print(df_rec2.index.tolist())
print(df_rec2.dtypes)